...
# identification of the problematic observations
...
# outliner points
ri <- rstandard(model)
outliner <- as.vector(which(ri > 4 | ri < -4))
...
for (i in c(2:7)){
  plot(x = coffee_df[, i], y = e_hat, xlab = names(coffee_df)[i],
       ylab = "Residual", main = paste("Residual vs. ", names(coffee_df)[i]))
}
mtext(~italic("Fig.2a: All residuals versus predictors plots from the model."), 
