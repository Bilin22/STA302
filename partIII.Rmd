...
skews in predictor variables, may be an issue in linearity
there is a severe left skew presented in the response (rating), indicating 
possible issues with linearity/ normality
```{r}
par(mfrow = c(3, 3))
for (i in c(2:8)) {
  hist(coffee_df[,i], main = "", xlab = names(coffee_df)[i])
}
```
...
```{r}
coffee_df2 <- coffee_df %>% filter(Ratings != 0)
...
# Re-check residual plots above with coffee_df2
```{r}
y_hat <- fitted(model2)
# residual
e_hat <- resid(model2)
...
# condition 1
```{r}
plot(x = y_hat, y = coffee_df2$Ratings, main = "Response vs. Fitted", xlab = "Fitted", 
     ylab = "Response")
abline(coef = c(0,1), lty = 2)
title(sub = ~italic("Fig.4: The response (coffee ratings) versus fitted values plot from the model."))
```
...
# Boxplot for species
```{r}
ggplot(data = coffee_df2, mapping = aes(x = Species, y = e_hat)) + 
  geom_boxplot() +
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), 
        plot.caption = element_text(hjust = 0.5, face = "italic"))+
...
plot(x = coffee_df2$ISpecies, y = e_hat,
                      main = "Residual vs. Species", xlab = "Species",
                      ylab = "Residual")
plot(x = coffee_df2$Aroma, y = e_hat,
                    main = "Residual vs. Aroma", xlab = "Aroma",
                    ylab = "Residual")
plot(x = coffee_df2$Flavor, y = e_hat,
                     main = "Residual vs. Flavor", xlab = "Flavor",
                     ylab = "Residual")
plot(x = coffee_df2$Aftertaste, y = e_hat,
                    main = "Residual vs. Aftertaste", xlab = "Aftertaste",
                    ylab = "Residual")
plot(x = coffee_df2$Acidity, y = e_hat,
                      main = "Residual vs. Acidity", xlab = "Acidity",
                      ylab = "Residual")

plot(x = coffee_df2$Sweetness, y = e_hat,
                    main = "Residual vs. Sweetness", xlab = "Sweetness",
                    ylab = "Residual")
mtext(~italic("Fig.2a: All residuals versus predictors plots from the model."), side = 1, line = 3, outer = TRUE, adj = 0.5)
...
# Pairwise plots of all predictors

```{r}
pairs(coffee_df2[, c(2:7)], main="Pairwise Scatterplots of Predictors")
title(sub = ~italic("Fig.5: The pairwise scatter plots of all predictors."))
