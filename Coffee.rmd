...
ggplot(data = coffee_df, mapping = aes(x = Species, y = e_hat)) + 
  ...
  labs(y = "Residuals", title = "Residual by Coffee Species", 
       caption = "Fig.4: Distribution of Residual vs. Coffee Species")
...
plot(x = coffee_df$Sweetness, y = e_hat,
                    ...
                    ylab = "Residual")
mtext(~italic("Fig.2: All residuals versus predictors plots from the model."), side = 1, line = 3, outer = TRUE, adj = 0.5)
...
title(sub = ~italic("Fig.5: The response (coffee ratings) versus fitted values plot from the model."))
...
title(sub = ~italic("Fig.6: The pairwise scatter plots of all predictors."))
